...
package com.rockdot.core.mvc {
	...
	import org.springextensions.actionscript.mvc.impl.CommandProxy;
	 ...
	 * @inheritDoc
	 */
	public class CoreController extends EventDispatcher implements IController, IApplicationContextAware {

		protected static const LOGGER:ILogger = getClassLogger(CoreController);
		...
		private var _failOnCommandNotFound:Boolean = false;
		...
		protected function removeListeners():void {
			StaticEventBus.removeEventClassListener(RockdotEvent, onMVCEventHandler);
		}
		...
		protected function addListeners():void {
			StaticEventBus.addEventClassListener(RockdotEvent, onMVCEventHandler);
		}
		 ...
		 */
		public function onMVCEventHandler(event:RockdotEvent):void {
			Assert.notNull(event, "event argument must not be null");
				LOGGER.debug("Handling MVCEvent {0}", [event]);
				var registrations:Vector.<CommandRegistration> = findCommandRegistrations(event);
				if (registrations != null) {
					...
					for each (var registration:CommandRegistration in registrations) {
						executeCommand(registration, event);
					}
				}
		}
