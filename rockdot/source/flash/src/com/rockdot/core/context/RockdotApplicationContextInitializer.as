...
package com.rockdot.core.context {
	 ...
	 * @author Roland Zwaga
	 */
	public class RockdotApplicationContextInitializer extends EventDispatcher implements IApplicationContextInitializer, IDisposable {
		...
		private static const LOGGER:ILogger = getClassLogger(RockdotApplicationContextInitializer);
		 ...
		 */
		public function RockdotApplicationContextInitializer(target:IEventDispatcher=null) {
			super(target);
		}
		...
		private function cleanUpObjectDefinitionCreation():void {
			...
			if (_applicationContext.propertiesProvider && _applicationContext.propertiesProvider.length > 0) {
				...
				placeholderConfig.properties = _applicationContext.propertiesProvider;
				
				/* We're disabling this postprocessor. It wreaks havoc on iOS performance */
...
//		}
		...
		private function propertyTextFilesLoadComplete(operationEvent:OperationEvent):void {
			...
			if (propertySources != null) {
				...
				propertiesParser ||= new RockdotKeyValuePropertiesParser();
