package com.rockdot.library.view.component.book.view {
	import com.rockdot.library.view.component.book.drawing.DrawingTool;
	import com.rockdot.library.view.component.book.geom.Geom;
	import com.rockdot.library.view.component.book.geom.Line;
	import com.rockdot.library.view.component.book.geom.SuperRectangle;
	import com.rockdot.library.view.component.book.utils.ArrayTool;
	import com.rockdot.library.view.component.book.utils.MathTool;
	...
	// CUSTOM:
		...
		public function drawFold   (graphics:Graphics, 
			...
			var area:SuperRectangle = new SuperRectangle(0, 0, this._page.book.getWidth()/2, this._page.book.getHeight());
		...
		protected function drawInsideHard  (graphics:Graphics, 
			...
			var strength:Number = (this._page.book.getWidth()/2 - Math.abs(area[0].x - area[1].x)) / (this._page.book.getWidth()/2) * this._solidColorAlpha;
		...
		protected function drawInsideSmooth    (graphics:Graphics, 
			...
			var point3:Point = new Point((this._page.book.getWidth()/2)*this._page.book.getLastFlippedCorner().x, this._page.book.getHeight()*this._page.book.getLastFlippedCorner().y);
			...
			var x:Number = (this._page.side == Page.LEFT) ? this._page.book.getWidth() : -this._page.book.getWidth()/2;
			var y:Number = (point2.y == 0) ? 0 : this._page.book.getHeight();
			newArea.push(new Point(x, y));
			y = (point2.y == 0) ? this._page.book.getHeight() : 0;
			newArea.push(new Point(x, y));
			...
			if (area.length == 3){
				...
				x = (this._page.side == Page.LEFT) ? 0 : this._page.book.getWidth()/2;
				newArea.push(new Point(x, y));
				newArea.splice(1, 0, area[0]);
				
				// if flipped corner is outside of the Book its boundaries:
				if (area[0].y < 0 || area[0].y > this._page.book.getHeight()){
			...
			if (area.length == 4){
				...
				if ((area[0].y < 0 && area[1].y < 0) || (area[0].y > this._page.book.getHeight() && area[1].y > this._page.book.getHeight())){
				...
				// calculate intersection (if only one flipped corner is out of the bounds of the Book):
					...
					if (line.a.y != hLine.a.y && line.b.y != hLine.a.y){
						intersection = Line.getIntersection(line, hLine);
						var spliceAt:uint;
						if (newArea[1].y < 0 || newArea[1].y > this._page.book.getHeight())	spliceAt = 0;
						if (newArea[2].y < 0 || newArea[2].y > this._page.book.getHeight())	spliceAt = newArea.length-4;
			...
			var strength:Number = Point.distance(point3, point4) / this._page.book.getWidth();
		...
		protected function drawOutsideHard (graphics:Graphics, 
			...
			var area0:Rectangle = new Rectangle(0, 0, this._page.book.getWidth()/2, this._page.book.getHeight());
			var area1:Rectangle = new Rectangle(0, 0, this._page.book.getWidth()/2, this._page.book.getHeight());
			area0.left	= area[0].x - this._page.book.getWidth()/2;
			area0.right	= Math.min(area[0].x, area[1].x);
			area1.left	= Math.max(area[0].x, area[1].x);
			area1.right	= area[0].x + this._page.book.getWidth()/2;
			
			// calculate strength of the gradients:
			var strength0:Number = 1 - (area[1].x - area0.x) / this._page.book.getWidth();
			var strength1:Number = Math.max(0, (area[1].x - area[0].x) / (this._page.book.getWidth()/2))
			
			// create the model for the gradients:
			var matrix0:Matrix = new Matrix();
			var matrix1:Matrix = new Matrix();
			matrix0.createGradientBox(this._page.book.getWidth()/2, this._page.book.getHeight(), 0, area0.left, 0);
			matrix1.createGradientBox(this._page.book.getWidth()/2, this._page.book.getHeight(), 0, area[0].x, 0);
		...
		protected function drawOutsideSmooth   (graphics:Graphics, 
			...
			if (!this._page.book.tearActive){
				...
				if (newArea[0].y == 0 || newArea[1].y == 0){
					newArea.push(new Point(this._page.side*this._page.book.getWidth()/2, 0));
				}else{
					newArea.push(new Point(this._page.side*this._page.book.getWidth()/2, this._page.book.getHeight()));
				}
				if (area.length == 4){
					var spliceAt:uint = (newArea[1].y == 0) ? 0 : newArea.length - 1;
					if (newArea[0].y == this._page.book.getHeight() || newArea[1].y == this._page.book.getHeight()){
						newArea.splice(spliceAt, 0, new Point(this._page.side*this._page.book.getWidth()/2, this._page.book.getHeight()));
					}
				}
			}
			...
			if (this._page.book.tearActive){
				newArea = ArrayTool.copy(area);
		 		if (newArea[0].y == 0){
					newArea[0].y = newArea[3].y = this._page.book.getHeight();
				}else{
			...
			var strength:Number;
			if (!this._page.book.tearActive){
				max = this._page.book.getWidth()/2;
				strength = 1 - Math.abs(max/2-Math.abs(point3.x-point4.x)) / (max/2);
			}else{
				max = this._page.book.getHeight();
		...
		public function drawFlipside   (graphics:Graphics, 
			...
			var max:Number = this._page.book.getWidth()/2;
		...
		protected function createNewMatrix (point1:Point, point2:Point, rotate:Number=Gradients.ROTATE_FULL):Matrix {
			...
			// define sizes:
			var larger:Number	= Math.max(this._page.book.getWidth()/2, this._page.book.getHeight());
			var smaller:Number	= Math.min(this._page.book.getWidth()/2, this._page.book.getHeight());
			if (larger == this._page.book.getHeight()){
		...
		protected function getOffset (point1:Point, point2:Point):Point {
			var middle:Point =	new Point(this._page.book.getWidth()/2/2, this._page.book.getHeight()/2);
