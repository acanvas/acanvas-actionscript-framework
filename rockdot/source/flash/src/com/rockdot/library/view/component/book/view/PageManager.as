package com.rockdot.library.view.component.book.view {
	import com.rockdot.library.view.component.book.containers.SuperViewStack;
	import com.rockdot.library.view.component.book.managers.StateManager;
	import com.rockdot.library.view.component.book.utils.ChildTool;
	 ...
	 * 
	 */
	public class PageManager extends RockdotManagedSpriteComponent {
		 ...
		 * @private
		 */
		protected var renderShape:Shape = new Shape();
	...
	// CONSTRUCTOR:
		 ...
		 * Constructor.
		 */
		public function PageManager (id : String):void {
			super(id);
...
//			this._pages.addEventListener(CollectionEvent.COLLECTION_CHANGE, this.onContentChanged);
	...
	// OVERRIDES:
		...
		override public function init (data : * = null) : void {
			...
			this.pageL.setSize(_width/2, _height);
			this.pageR.setSize(_width/2, _height);
			
			this.pageL.fade = this.pageR.fade = 0;
			this.pageR.x = this.renderShape.x = _width / 2;
		...
		override public function destroy ():void {
			...
			for (var i:int=0; i<this._pages.length; i++){
				Page(this._pages[i]).setBook(null);
			}
			// reset properties:
		...
		override public function removeChild (child:DisplayObject):DisplayObject {
			...
			if (child is Page){
				var index:int = this._pages.indexOf(child);
		...
		override public function removeChildAt (index:int):DisplayObject {
			...
			// define Page:
			var page:Page = Page(this._pages[index]);
		...
		override public function swapChildrenAt (index1:int, index2:int):void {
			this.swapChildren(Page(this._pages[index1]), Page(this._pages[index2]));
		}
	...
	// COMMON USAGE:
		...
		protected function generateStackIndex (page:Page):int {
			var index:int = this._pages.indexOf(page);
			if (index%2 == 1){
		...
		protected function refreshViewStacks ():void {
			...
			if (this.pageL.visible){
				this.pageL.selectedChild = Sprite(this._pages[this._currentPage]);
			}
			this.pageL.render();
			
			this.pageR.visible = ((!this.isLastPage(this._currentPage) || this._pages.length <= 1) && this.pageR.numChildren > 0);
			if (this.pageR.visible){
				this.pageR.selectedChild = Sprite(this._pages[this._currentPage+1]);
			}
			this.pageR.render();
