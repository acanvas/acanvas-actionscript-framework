package com.rockdot.library.view.component.book {
	import com.rockdot.library.view.component.book.flashsandy.DistortImage;
	import com.rockdot.library.view.component.book.foxaweb.PageFlip;
	import com.rockdot.library.view.component.book.geom.Geom;
	import com.rockdot.library.view.component.book.managers.StateManager;
	import com.rockdot.library.view.component.book.view.BookError;
	import com.rockdot.library.view.component.book.view.BookEvent;
	import com.rockdot.library.view.component.book.view.Gradients;
	import com.rockdot.library.view.component.book.view.Page;
	import com.rockdot.library.view.component.book.view.PageManager;
	import com.rockdot.library.view.component.book.view.limited;
	 ...
	 * 
	 */
	public class BookView extends PageManager {
	...
	// CONSTRUCTOR:
		 ...
		 * Constructor.
		 */
		public function BookView (id : String):void {
			super(id);
...
//			 set initial status:
	...
	// OVERRIDES:
		...
		override public function init (data : * = null) : void {
			...
			for (var i:int=0; i<this._pages.length; i++){
				Page(this._pages[i]).refreshFoldGradient();
			}
		...
		override public function render():void {
			...
			this.distortion = new DistortImage(_width/2, _height);
		}
		...
		override protected function jumpViewStacks (fromIndex:int):void {
			...
			for (var i:int=fromIndex; i<this._pages.length; i++){
				Page(this._pages[i]).refreshFoldGradient(false);
			}
		}
	...
	// PAGEFLIP PROCESSING:
		...
		protected function startPageFlip (event:MouseEvent=null, attemptHover:Boolean=false):void {
			...
			if (this.autoFlipActive && !this.tearActive && this.pageCorner.x >= 0 && this.pageCorner.x <= _width/2 && event != null){
			...
			if (Page(this._pages[this._currentPage+side]).lock){
			...
			this.renderShape.x = this.lastFlippedSide * _width/2;
			...
			// save bitmapData:
			this.saveBitmapData(Page(this._pages[frontIndex]), Page(this._pages[backIndex]));
			...
			if (this.lastFlippedSide == Page.LEFT){ // if left-hand page was flipped
				...
				if (this.pageL.visible){
					this.pageL.selectedChild = Sprite(this._pages[this._currentPage-2]);
				}else{
				}
			}
			if (this.lastFlippedSide == Page.RIGHT){ // if right-hand page was flipped
				...
				if (this.pageR.visible){
					this.pageR.selectedChild = Sprite(this._pages[this._currentPage+1+2]);
				}
			}
			
			// set page corner markers:
			this.pageCorner = new Point(this.lastFlippedCorner.x*_width/2, this.lastFlippedCorner.y*_height);
			...
			// dispatch event:
			var page:Page = Page(this._pages[this._currentPage+this.lastFlippedSide]);
			if (this.hoverActive){
		...
		protected function dragPageCorner (event:Event=null):void {
			...
			var frontIndex:uint	= this._currentPage + this.lastFlippedSide;
			var backIndex:uint	= this._currentPage + this.lastFlippedSide + this.lastFlippedDirection;
			var front:Page	= Page(this._pages[frontIndex]);
			var back:Page	= Page(this._pages[backIndex]);
...
//			}
			...
			if (!front.explicitHard && !back.explicitHard){
				...
				if (this._status == BookEvent.PAGEFLIP_STARTED && Page(this._pages[this._currentPage+this.lastFlippedSide]).tearable && ocf.cPoints != null){
		...
		protected function endPageFlip (event:MouseEvent=null):void {
			...
			if (!this.tearActive){
				var x:Number;
				var y:Number = this.lastFlippedCorner.y*_height;
				if (this.lastFlippedSide == Page.LEFT){ // if left-hand page was flipped
					this.lastFlipSucceeded = (this.pageCornerTarget.x > _width/2);
					x = (this.lastFlipSucceeded) ? _width : 0;
				}else{ // if right-hand page was flipped
					this.lastFlipSucceeded = (this.pageCornerTarget.x < 0);
					x = (this.lastFlipSucceeded) ? -_width/2 : _width/2;
				}
		...
		protected function drawHardPage (front:BitmapData, back:BitmapData):void {
			...
			if (this.lastFlippedDirection == 1){
				w = this.pageCorner.x - (1-this.lastFlippedSide) * _width/2;
			}else{
				w = this.pageCorner.x - _width/2;
			}
			if (w < -_width/2) w = -_width/2;
			if (w >  _width/2) w =  _width/2;
			
			// calculate perspective:
			var closeness:Number = Math.sin(Math.acos(w / (_width/2)));
			
			// define positions of page-corners:
			var pPoints:Array = [];
			pPoints[0] = new Point((1-this.lastFlippedSide)*_width/2, 0);
			pPoints[1] = new Point(pPoints[0].x+w, 0-closeness*this.hardPerspective);
			pPoints[2] = new Point(pPoints[0].x+w, _height+closeness*this.hardPerspective);
			pPoints[3] = new Point((1-this.lastFlippedSide)*_width/2, _height);
			...
			if (this.lastFlippedSide == 0){
				bmd = (this.pageCorner.x > _width/2) ? back : front;
			}else{
		...
		protected function evaluateHover (event:Event=null):void {
			...
			if (Page(this._pages[this._currentPage+side]).hard){
				return;
			}
		...
		protected function evaluateTear (point:Point):void {
			...
			// evaluate:
			if (Math.round(point.x) == (1-this.lastFlippedCorner.x) * _width/2 && Math.round(point.y) == this.lastFlippedCorner.y * _height){
	...
	// GRADIENTS:
		...
		protected function addHardGradients (area:Array):void {
			...
			var tint:String;
			if (this.lastFlippedSide == Page.LEFT){ // if left-hand page was flipped
				tint = (this.pageCornerTarget.x > _width/2) ? Gradients.DARK : Gradients.LIGHT;
			}else{ // if right-hand page was flipped
	...
	// PAGEFLIP ASSISTANCE:
		...
		protected function movePageCornerTarget ():Boolean {
			...
			if (!this.autoFlipActive){
				...
				x = this.mouseX;
				y = (!this.sideFlipActive) ? this.mouseY : this.lastFlippedCorner.y * _height;
				...
				if (this.sideFlipActive){
					...
					var xMin:Number = 0 - this.lastFlippedSide * (_width/2);
					var xMax:Number = xMin + _width;
					if (x < xMin) x = xMin;
					if (x > xMax) x = xMax;
			...
			if (this.autoFlipActive){
				// calculate coordinates:
				x = (!this.tearActive) ? this.pageCornerTarget.x : this.lastFlippedSide * (_width/2);
				y = this.pageCornerTarget.y;
				var opposite:Point = new Point();
				// if this is a normal auto-flip:
				if (!this.tearActive){
					opposite.x = this.lastFlippedSide * (_width/2) - _width*this.lastFlippedDirection;
				...
				if (this.tearActive){
					...
					opposite.y = (_height/2) + (_height*1.5 * directionY);
		...
		protected function movePageCorner ():void {
			...
			if ((this._status == BookEvent.PAGEFLIP_ENDING || this._status == BookEvent.HOVER_ENDING) && !this.tearActive){
				...
				if (this.pageCorner.y > _height)	this.pageCorner.y = _height;
			}
		...
		protected function getCurrentCorner ():Point {
			...
			// determine corner:
			corner.x = (this.mouseX < _width/2 ) ? 0 : 1;
			corner.y = (this.mouseY < _height/2) ? 0 : 1;
			// return value:
			return corner;
		}
		...
		protected function getCurrentSide ():uint {
			var side:uint;
			if (!this.autoFlipActive){
				side = (this.mouseX <= _width/2) ? Page.LEFT : Page.RIGHT;
			}else{
				if (!this.tearActive){
		 ...
		 * @private
		 */
		protected function getPageCornerYFromX (x:Number, curve:Number=BookView.AUTO_FLIP_CURVE):Number {
			var middleX:Number	= this.lastFlippedSide * (_width/2) - (_width/2)*this.lastFlippedDirection;
 			var progress:Number	= Math.abs(middleX-x) / (_width/2);
			var y:Number;
			if (this.lastFlippedCorner.y == 0){
				y = _height * curve * (1-progress);
			}else{
				y = _height * (1-curve) + progress * (_height*curve);
			}
			return y;
		}
	...
	// ACCESSORS:
		...
		protected function get flipOnRelease ():Boolean {
			if (this.flipOnClick && !this.autoFlipActive){
				return (getTimer() - this.lastFlippedTime <= BookView.CLICK_INTERVAL);
			}else{
				return false;
			}
		}
		...
		protected function get autoFlipSpeed ():uint {
			return _width / ((this.autoFlipDuration/1000) * this.stage.frameRate);
		}
		...
		public function set hardCover (value:Boolean):void {
			...
			if (StateManager.instance.getState(this) >= StateManager.CREATION_COMPLETE){
				Page(this._pages[0]).refreshFoldGradient();
				Page(this._pages[this._pages.length-1]).refreshFoldGradient();
			}
		}
		...
		protected function createRegions ():void {
			...
			this.regions[0].TR.x += _width / 2 - this.regionSize;
			this.regions[0].BR.x += _width / 2 - this.regionSize;
			this.regions[0].BR.y += _height - this.regionSize;
			this.regions[0].BL.y += _height - this.regionSize;
			// specify regions for right-hand page:
			this.regions[1] = {};
			this.regions[1].TL = new Rectangle(_width/2, 0, this._regionSize, this._regionSize);
			this.regions[1].TR = new Rectangle(_width/2, 0, this._regionSize, this._regionSize);
			this.regions[1].BR = new Rectangle(_width/2, 0, this._regionSize, this._regionSize);
			this.regions[1].BL = new Rectangle(_width/2, 0, this._regionSize, this._regionSize);
			this.regions[1].TR.x += _width / 2 - this.regionSize;
			this.regions[1].BR.x += _width / 2 - this.regionSize;
			this.regions[1].BR.y += _height - this.regionSize;
			this.regions[1].BL.y += _height - this.regionSize;
			// specify side-flip regions for both pages:
			this.regions[0].side = new Rectangle   (0,
													(_height-this._regionSize)/2,
													this._regionSize/2,
													this._regionSize);
			this.regions[1].side = this.regions[0].side.clone()
			this.regions[0].side.x = _width-this._regionSize/2;
		}
