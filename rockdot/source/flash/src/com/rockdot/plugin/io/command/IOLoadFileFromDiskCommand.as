package com.rockdot.plugin.io.command {
	...
	public class IOLoadFileFromDiskCommand extends AbstractIOCommand {
		...
		override public function execute(event : RockdotEvent = null) : * {
			...
			if (File.userDirectory.resolvePath("Pictures").exists) {
				_file = File.userDirectory.resolvePath("Pictures");
			} else if (File.userDirectory.resolvePath("My Pictures").exists) {
				_file = File.userDirectory.resolvePath("My Pictures");
			} else {
				_file = File.userDirectory;
			}
			_file.browseForOpen("Choose a picture", [imageFileTypes]);
