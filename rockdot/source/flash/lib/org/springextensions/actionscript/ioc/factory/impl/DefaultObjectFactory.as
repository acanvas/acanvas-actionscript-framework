...
package org.springextensions.actionscript.ioc.factory.impl {
	...
	public class DefaultObjectFactory extends EventDispatcher implements IObjectFactory, IEventBusAware, IAutowireProcessorAware, IDisposable, IEventBusUserRegistryAware {
		...
		private function attemptToInstantiate(objectDefinition:IObjectDefinition, constructorArguments:Array, name:String, objectName:String):* {
			var result:* = null;
			try {
		...
		private function buildObject(name:String, constructorArguments:Array):* {
			var result:*;
			var isFactoryDereference:Boolean = (name.charAt(0) == OBJECT_FACTORY_PREFIX);
			var objectName:String = (isFactoryDereference ? name.substring(1) : name);
			...
			if ((objectDefinition.scope !== ObjectDefinitionScope.SINGLETON) && (objectDefinition.scope !== ObjectDefinitionScope.PROTOTYPE)) {
				throw new ObjectFactoryError(StringUtils.substitute("Only definitions with scope 'singleton' or 'prototype' can be instantiated. Definition name: {0}", objectName));
			}
		...
		private function instantiateClass(objectDefinition:IObjectDefinition, constructorArguments:Array, objectName:String):* {
