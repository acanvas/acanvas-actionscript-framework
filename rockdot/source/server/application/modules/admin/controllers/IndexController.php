...
class Admin_IndexController extends Zend_Controller_Action{
	...
	*/
	...
	public function init(){
		try{
			JVM_Debug::dump( $this->_request->getParams(), 'REQUEST_INIT' );
			//----------------------------------------------------------------------
			//switch layout
			...
			JVM_Debug::dump($e->getMessage(), 'EXCEPTION@init()');
		}
	}
	...
	public function preDispatch(){
		try{
			//----------------------------------------------------------------------
			//basic ip-protection
			if(!in_array(JVM_Http_Request::getIp(), Zend_Registry::get('Application_Config')->page->admin_ips->toArray())){
	...
	public function indexAction(){
		try{
			//init index_session
			$session = new Zend_Session_Namespace('JVM_Admin_Controller_Index_Toplist');
			//-----------------------------------------------
			//init profile model
			$profile = new JVM_Model_Admin(
					array( 'metadataCache' => Zend_Registry::get('Cache') )
			);
			//-----------------------------------------------
			// sortby
	...
	public function userlistAction(){
		try{
			//init index_session
			$session = new Zend_Session_Namespace('JVM_Admin_Controller_Index');
			//-----------------------------------------------
			//init profile model
			$profile = new JVM_Model_Admin(
					array( 'metadataCache' => Zend_Registry::get('Cache') )
			...
			if($count > 0){
				...
				$this->view->showPerPage = new JVM_Form_PerPage(
						array(
								'config' =>	array(
										'perpage' => array(
												'options' => array(
