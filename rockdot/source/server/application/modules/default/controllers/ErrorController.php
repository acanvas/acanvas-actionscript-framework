	...
	*/	
    public function errorAction(){
		...
		try{
			...
			//-----------------------------------------------------------------------------------------
			//navigation
			$navigation = Zend_Controller_Front::getInstance()->getPlugin('JVM_Zend_View_Helper_Page');
			if(!$navigation instanceof JVM_Zend_View_Helper_Page){
	...
	private function dumpError(){
		...
		if(APPLICATION_ENV !== 'production'){
			...
			if(isset($this->_errors->request) && strstr($dump, 'full')){ 
				JVM_Debug::dump($this->_errors, 'ERROR_OBJECT::FULL');
			}
			else{
				if(isset($this->_errors->exception)){
					...
					if(strstr($dump, 'exception')){
						JVM_Debug::dump($this->_errors->exception, 'ERROR_OBJECT::EXCEPTION');
					}
				}
				if(isset($this->_errors->request) && strstr($dump, 'request')){
					JVM_Debug::dump($this->_errors->request, 'ERROR_OBJECT::REQUEST');	
				}
			}
		}
		//-----------------------------------------------------------------------------------------	
