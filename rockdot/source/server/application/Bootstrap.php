...
*
* @author _________joerg.diterlizzi
* @version:________1.0
* @lastmodified:___$Date: $
* @revision:_______$Revision: $
* @copyright:______Copyright (c) Jung von Matt / Neckar
*
...
* APPLICATION_PATH_LIBRARY //application path to libraries (Zend | JVM | Tools)
...
class Bootstrap extends Zend_Application_Bootstrap_Bootstrap{
	...
	private function initAutoload(){
		try{
			...
			$autoloader->registerNamespace('JVM');
		}
		catch(Exception $e){
	...
	private function initSession(){
		try{
			...
			$this->sessionNamespace = new Zend_Session_Namespace('JVM_System');
	...
	private function initSystemRequests(){
		try{
			...
			$this->request->isGet() /* &&
			in_array(JVM_Http_Request::getIp(), Zend_Registry::get('Application_Config')->page->admin_ips->toArray()) */
			){
		...
		try{
			if($this->isMultiLanguage()){
				...
				*/
				...
				$jvm_registry->set('fb_sig_data', $data);
				$jvm_registry->set('country', substr($locale, 3, 2));
				$jvm_registry->set('language', $language);
				$jvm_registry->set('locale', $locale);
				$jvm_registry->set('protocol', $protocol);
				$jvm_registry->set('app_data', $app_data);
	...
	private function initLocale(){
		try{
			...
			$locale = new Zend_Locale($jvm_registry->get('locale') );
	...
	private function initPluginsAndHelpers(){
		try{
			...
			new JVM_Zend_Plugin_LoginStatus()
			);
			//---------------------
			//register Rewrite/Mock Current IP Adress
			Zend_Controller_Front::getInstance()->registerPlugin(
			new JVM_Zend_Plugin_RewriteIp()
			);
			//---------------------
			//register Permanent-SSL-Switch
			Zend_Controller_Front::getInstance()->registerPlugin(
			new JVM_Zend_Plugin_SslSwitch(Zend_Registry::get('Application_Config')->page->sslenvironments)
			);
			//---------------------
			//register MobileDetection-Plugin
			Zend_Controller_Front::getInstance()->registerPlugin(
			new JVM_Zend_Plugin_DetectMobile()
			);
			//---------------------
			...
			if(isset(Zend_Registry::get('Application_Config')->page->maintenance) && Zend_Registry::get('Application_Config')->page->maintenance === 'on'){
				Zend_Controller_Front::getInstance()->registerPlugin(new JVM_Zend_Plugin_Maintenance());
			}
			//---------------------
			//global available ViewHelper
			$navigationHelper = new JVM_Zend_View_Helper_Page(
					array(
			...
			Zend_Layout::getMvcInstance()->getView()->registerHelper(new JVM_Zend_View_Helper_FormText(), 'FormText');//(renders html5 form elements) //if doctype XHTML5
			Zend_Layout::getMvcInstance()->getView()->registerHelper(new JVM_Zend_View_Helper_Script(), 'script');	//footerscript parent of headscript
		}
		catch(Exception $e){
		...
		try{
			...
			if (!$result->isEmpty()){
				...
				if(count($_iterator) > 0 && $impact > 10){
					if(APPLICATION_ENV !== 'production'){
						JVM_Debug::dump($result, 'IMPACT:'. $impact);
					}
