package com.rockdot.project.view.layer {
	...
	public class AbstractLayer extends RockdotManagedSpriteComponent implements IModelAware{
		...
		override public function init(data : * = null) : void {
			...
			// kamera
			_kamera = Assets.home_imagecloud;
			addChild(_kamera);

			// logo
			if ( RockdotConstants.MARKET == Markets.CZECH_REPUBLIC ) {
				_logo = Assets.logo_i_am_nikon_cz;
			} else if ( RockdotConstants.MARKET == Markets.RUSSIA ) {
				_logo = Assets.logo_i_am_nikon_ru;
			} else {
				_logo = Assets.logo_i_am_nikon;
			}
			addChild(_logo);
		...
		override public function render() : void {
			...
			_kamera.x = _width/2 - _kamera.width/2;
			_kamera.y = 35;
			
			_logo.x = _kamera.x + 230;
			_logo.y = 15;
