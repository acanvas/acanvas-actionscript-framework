package com.rockdot.project.view.layer {
	...
	public class PreLogin extends AbstractLayer {
		...
		private var _login : SimpleButton;
		private var _cancel : SimpleButton;
		...
		override public function init(data : * = null) : void {
			...
			_login = new SimpleButton(getProperty("button.login"), _width, BootstrapConstants.HEIGHT_RASTER);
			_login.submitCallback = _onLoginClick;
			addChild(_login);

			// closes this layer
			_cancel = new SimpleButton(getProperty("button.back"), _width, BootstrapConstants.HEIGHT_RASTER);
		...
		private function _onLoginClick() : void {
			...
			new RockdotEvent(ProjectExtendedEvents.APP_LOGIN, {prompt:_data.prompt, next:_data.next}, _onLogin).dispatch();
		}
		...
		private function _onLogin(dao : UGCUserVO) : void {
			...
			new RockdotEvent(ProjectExtendedEvents.APP_AFTERLOGIN_DISPATCH, _data).dispatch();
